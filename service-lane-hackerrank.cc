/*
 * Calvin was driving his favorite vehicle on the 101 freeway. He noticed the check engine light was on and wants to service it immediately  * to avoid any risks. Luckily, a service lane runs parallel to the highway. The length of the highway and service lane is N units. The serv * ice lane consists of N segments of unit length, where each segment can have different widths.
 *
 * Calvin can enter into and exit from any segment. Let’s call the entry segment as index i and the exit segment as index j. Assume that the * exit segment lies after the enter segment(j>i) and i ≥ 0. Calvin has to pass through all segments whose index lies between i and j (both  * in  clusive).
 *
 * Paradise Highway
 *
...
int main()
{
    ...
    for(int i=0; i<T; i++)
    {
        cin >>enters[i]>>exits[i];
        multiset<int> lSet;
        for(int j=enters[i];j<=exits[i];j++)
            lSet.insert(width[j]);

        //note we are using the ordered property of a mulitset, where begin() gives the smallest element
        cout<<*lSet.begin()<<endl;
    }
